...
def train(configs):
    ...
    random.seed(1234)
    wandb.init(project="IMU2CLIP", config=configs, name="Spatial Attention s7")
    ...
    if "video" in list_modalities:
        ...
        video_encoder = (
            ClipPLModel(freeze=True, use_segm_mask=True) if text_encoder is None else text_encoder
        )
